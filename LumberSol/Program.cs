...
namespace _07Lumber
{
    ...
    public class Lumber
    {
        ...
        public static void Main()
        {
            ...
            {
                int j = i + 1;
                if (!logsGraph.ContainsKey(logs[i].LogNumber))
                    logsGraph.Add(logs[i].LogNumber, new List<int>());

                while (j < logs.Count && logs[j].X1 <= logs[i].X2)
                {
                    if (logs[j].Y1 >= logs[i].Y2 && logs[j].Y2 <= logs[i].Y1)
                    {
                        if (!logsGraph.ContainsKey(logs[j].LogNumber)) 
                            logsGraph.Add(logs[j].LogNumber, new List<int>());
